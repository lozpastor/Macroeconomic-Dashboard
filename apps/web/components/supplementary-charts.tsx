...
import { analysisPair, matchedPairs, matchedSpread, momentumMetrics, periodChanges, previousPeriod, deviationScale, deviationColor, deviationPalette } from "@/lib/supplementary";
...
const colorCopy = {
  es: { low: "Inferior", middle: "Mediana", high: "Superior", note: "Color y tamano: distancia a la mediana del eje Y en esta muestra; no es una valoracion de riesgo o bienestar." },
  en: { low: "Lower", middle: "Median", high: "Higher", note: "Color and size: distance from the sample median on the Y axis; not a risk or welfare rating." },
  zh: { low: "较低", middle: "中位数", high: "较高", note: "颜色和大小表示与本样本Y轴中位数的距离，并非风险或福利评级。" }
};
const panelClass = "min-w-0 rounded-md border border-stone-200 bg-white p-5";
const headingClass = "text-xs font-medium uppercase tracking-[0.18em] text-stone-500";
const tooltip = { trigger: "item" as const, renderMode: "richText" as const, confine: true, borderWidth: 0, backgroundColor: "rgba(28,31,28,0.92)", textStyle: { color: "#f4f3ee", fontSize: 12 } };
const axisStyle = { axisLine: { lineStyle: { color: "#dcdbd2" } }, axisLabel: { color: "#8a8d86", fontSize: 11 }, splitLine: { lineStyle: { color: "#ecebe3" } } };
...
export function CountryAnalysis({ rows, metric, freq, period, factor, currency }: {
  ...
  const scale = deviationScale(points.map(p => p.y));
  const colors = colorCopy[lang];
  ...
  const pairOption: EChartsOption = pair ? {
    ...
    series: [{ type: "scatter", emphasis: { scale: 1.3, itemStyle: { opacity: 1, borderColor: "#1c1f1c", borderWidth: 1.5 } }, data: points.map((p) => {
      const score = scale.score(p.y);
      return { value: [scaled(p.x, pair[0]), scaled(p.y, pair[1])], symbolSize: 7 + Math.abs(score) * 7, itemStyle: { color: deviationColor(score), opacity: 0.5 + Math.abs(score) * 0.5, borderWidth: 0.5, borderColor: "#ffffff" } };
    }),
  ...
  return <section data-testid="country-analysis" className={`mt-5 grid gap-5 ${hasPair && changes.length ? "xl:grid-cols-2" : ""}`}>
    {hasPair && pair && <article className={panelClass} data-testid="paired-analysis">
      <h2 className={headingClass}>{text.pair}: {tr.mShort(pair[0])} / {tr.mShort(pair[1])}</h2>
      ...
      <div data-testid="scatter-legend" className="mt-3 max-w-sm text-[11px] text-stone-500">
        <div className="h-1.5 rounded-sm" style={{ background: `linear-gradient(to right, ${deviationPalette.low}, ${deviationPalette.middle} 50%, ${deviationPalette.high})` }} />
        <div className="mt-1 flex justify-between gap-2"><span>{colors.low}</span><span>{colors.middle}: {n(scale.median)} {unit(pair[1])}</span><span>{colors.high}</span></div>
      </div>
      <p className="mt-2 text-xs leading-relaxed text-stone-500">{colors.note}</p>
      <p className="mt-2 text-xs text-stone-500">{pair[0] === "publicDebt" ? text.fiscal : pair[0] === "cpiCore" ? text.cpi : text.relation}</p>
    </article>}
    {!!changes.length && <article className={panelClass} data-testid="change-analysis">
      <h2 className={headingClass}>{text.changes} · {deltaUnit}</h2>
...
export function GlobalSpread({ global, metric, factor, currency }: { global: GlobalIndicators; metric: MetricKey; factor: number; currency: string }) {
  ...
  return <section data-testid="global-spread" className={`mt-5 ${panelClass}`}>
    <h2 className={headingClass}>{oil ? text.oil : text.rates} · {unit}{oil ? "/bbl" : ""}</h2>
